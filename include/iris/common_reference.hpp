// https://cpplover.blogspot.com/2016/01/c-p0022r1-p0092r1.html
// https://faithandbrave.hateblo.jp/entry/20100305/1267779325
#include <type_traits>
#include <utility>
/*
template <class T>
...
template <class T>
requires _Valid<__t, T> struct __has_type<T> : std::true_type {};
...
using __rref_res = std::conditional_t<__v<std::is_reference<R>>, std::remove_reference_t<R>&&, R>;
template <class T, class U>
...
template <class T, class U>
requires _Valid<__cond_res, __cref<T>, __cref<U>> struct __builtin_common<T, U>
  : std::decay<__cond_res<__cref<T>, __cref<U>>> {};
template <class T, class U>
requires _Valid<__builtin_common_t, T&, U&>&& _ConvertibleTo<T&&, __rref_res<T, U>>&&
  _ConvertibleTo<U&&, __rref_res<T, U>> struct __builtin_common<T&&, U&&> : __id<__rref_res<T, U>> {
};
template <class T, class U>
...
requires _Valid<__builtin_common_t, T&, U const&>&&
  _ConvertibleTo<U&&, __builtin_common_t<T&, U const&>> struct __builtin_common<T&, U&&>
...
template <class T>
concept bool _Decayed = __v<std::is_same<std::decay_t<T>, T>>;
...
template <class T, class U>
using __basic_common_reference = basic_common_reference<__uncvref<T>,
                                                        __uncvref<U>,
...
requires _Valid<__builtin_common_t, T, U>&&
  _Is<__builtin_common_t<T, U>, std::is_reference> struct __common_reference2<T, U>
