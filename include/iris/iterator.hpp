...
#define IRIS_DEFINE_ITERATOR_TRAITS_TYPE_ALIASES_1234                                              \
  ...
  static pointer to_address(iterator_type const& iter) noexcept { return iter.operator->(); }
#define IRIS_DEFINE_ITERATOR_TRAITS_COMMON_12345                                                   \
  ...
  template <                                                                                       \
    typename X,                                                                                    \
    enable_if_t<std::is_assignable_v<detected_t<indirection_t, iterator_type&>, X const&>> =       \
      nullptr>                                                                                     \
  ...
  template <typename X,                                                                            \
            enable_if_t<std::conjunction_v<                                                        \
              ...
              std::is_assignable<detected_t<indirection_t, iterator_type&>, X const&>>> = nullptr> \
  static void assign(iterator_type const& iter, X&& x) {                                           \
    *iter = std::move(x);                                                                          \
  }
#define IRIS_DEFINE_ITERATOR_TRAITS_EQUAL_TO_ETC_2345                                              \
  static bool equal_to(iterator_type const& lhs, iterator_type const& rhs) { return lhs == rhs; }  \
  static bool not_equal_to(iterator_type const& lhs, iterator_type const& rhs) {                   \
...
#define IRIS_DEFINE_ITERATOR_TRAITS_LESS_ETC_45                                                    \
  static bool less(iterator_type const& lhs, iterator_type const& rhs) { return lhs < rhs; }       \
  ...
  static bool greater_equal(iterator_type const& lhs, iterator_type const& rhs) {                  \
  ...
  struct iterator_traits<
    I,
    std::enable_if_t<std::conjunction_v<is_input_iterator_class<I>,
  ...
  struct iterator_traits<
    I,
    std::enable_if_t<std::conjunction_v<is_forward_iterator_class<I>,
    ...
    static difference_type distance(iterator_type lhs, iterator_type const& rhs) {
      difference_type result{0};
      for (; iterator_traits::not_equal_to(lhs, rhs); iterator_traits::advance(lhs)) ++result;
      return result;
    }
  };
  ...
  struct iterator_traits<
    I,
    std::enable_if_t<std::conjunction_v<is_bidirectional_iterator_class<I>,
  ...
  // 4
  template <typename I>
  struct iterator_traits<I, std::enable_if_t<is_random_access_iterator_class_v<I>>> {
    ...
    static iterator_type& advance(iterator_type& iter, difference_type n = 1) { return iter += n; }
    static difference_type distance(iterator_type const& lhs, iterator_type const& rhs) {
      return rhs - lhs;
    }
  };
  ...
  struct iterator_traits<T*, std::enable_if_t<std::is_object_v<T>>> {
    ...
    IRIS_DEFINE_ITERATOR_TRAITS_RETREAT_PREV_345
    IRIS_DEFINE_ITERATOR_TRAITS_LESS_ETC_45
    static pointer to_address(iterator_type const& iter) noexcept { return iter; }
    static iterator_type& advance(iterator_type& iter, difference_type n = 1) { return iter += n; }
    static difference_type distance(iterator_type const& lhs, iterator_type const& rhs) {
      return rhs - lhs;
    }
